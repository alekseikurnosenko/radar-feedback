...
const reducer = (state: State, action: Action): State => {
    const questionByAnswer = (answerId: string) =>
        state.questions.find((q) => q.answers.some((a) => a.id === answerId))!;
    ...
    return (
        <div className="flex flex-col p-2 mt-2 bg-blue-100 border border-black">
            ...
            <Button
                ...
                onClick={() =>
                    dispatch({ type: 'suggestionRemoved', answerId: answer.id, suggestionId: suggestion.id })
                }
            >
                Remove
            </Button>
    ...
    return (
        <div className="flex flex-col grid-rows-4 p-2 mt-2 bg-green-100 border border-black">
            ...
            <Input
                value={answer.text}
                onChange={(e) =>
                    dispatch({ type: 'answerChanged', answerId: answer.id, payload: { text: e.target.value } })
                }
            />
            ...
            <select
                className="p-2"
                onChange={(e) =>
                    dispatch({ type: 'answerChanged', answerId: answer.id, payload: { measurement: e.target.value } })
                }
            >
                {measurements.map((m) => (
            ...
            <div className="flex flex-row items-center">
                ...
                <Button className="ml-auto" onClick={() => dispatch({ type: 'suggestionAdded', answerId: answer.id })}>
                    +
                </Button>
            </div>
            ...
            <Button className="ml-auto mt-2" onClick={() => dispatch({ type: 'answerRemoved', answerId: answer.id })}>
                Remove
            </Button>
        </div>
    );
    ...
    return (
        <div className="flex flex-col p-4 mb-4 bg-white border border-black">
            ...
            <Input
                value={question.text}
                onChange={(e) =>
                    dispatch({ type: 'questionChanged', newText: e.target.value, questionId: question.id })
                }
            />
            <div className="flex flex-row items-center">
                <p className="mt-2">Answers</p>
                <Button className="ml-auto" onClick={() => dispatch({ type: 'answerAdded', questionId: question.id })}>
                    +
                </Button>
            </div>
            {question.answers.map((a) => (
                <AnswerBlock key={a.id} answer={a} measurements={measurements}></AnswerBlock>
            ))}
            <Button
                className="ml-auto mt-2"
        ...
        <DispatchContext.Provider value={dispatch}>
            <div className="flex flex-col p-8 bg-background">
                ...
                <div className="flex flex-row mb-2 items-center">
                    ...
                    <Button className="ml-auto" onClick={() => dispatch({ type: 'questionAdded' })}>
                        +
                    </Button>
                </div>
                {state.questions.map((q) => (
                    <QuestionBlock key={q.id} question={q} measurements={measurements} />
                ))}
            </div>
